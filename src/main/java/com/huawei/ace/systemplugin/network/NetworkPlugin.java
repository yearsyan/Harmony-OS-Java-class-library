...
public class NetworkPlugin implements ModuleGroup.ModuleGroupHandler, EventGroup.EventGroupHandler {
    ...
    private boolean hasPremission() {
        if (!this.hasCheckPermission) {
            this.hasPermission = this.applicationContext.verifySelfPermission("ohos.permission.GET_NETWORK_INFO") == 0 && this.applicationContext.verifySelfPermission("ohos.permission.GET_WIFI_INFO") == 0;
    ...
    public void onFunctionCall(Function function, Result result) {
        if (!"getNetworkType".equals(function.name)) {
            ...
            jSONObject.put("type", (Object) this.commonEventSubscriber.getNetworkType());
            jSONObject.put("metered", (Object) Boolean.valueOf(this.commonEventSubscriber.isMetered()));
    ...
    public void onSubscribe(List<Object> list, EventNotifier eventNotifier2, Result result) {
        ...
        if (this.isSubscribeSuccess) {
            ...
            this.commonEventSubscriber.onReceiveEvent(null);
            return;
        }
