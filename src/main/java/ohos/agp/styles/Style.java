...
public class Style {
    ...
    public static int getPatternId(Context context, String str) {
        ...
        try {
            ...
            if (!(theme == null || (typedAttribute = theme.getThemeHash().get(str)) == null || typedAttribute.getType() != 7)) {
                try {
                    return typedAttribute.getResId();
                } catch (IOException | NotExistException | WrongTypeException e) {
                    HiLog.error(TAG, "Get pattern resource id failed. ", e.getMessage());
                }
            }
            return 0;
        } catch (IllegalArgumentException e2) {
            HiLog.warn(TAG, "Get theme failed! ", e2.getMessage());
            return 0;
        }
    }
