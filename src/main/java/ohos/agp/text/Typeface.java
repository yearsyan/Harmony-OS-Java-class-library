...
public class Typeface {
    ...
    private static final Map<String, Typeface> APP_TYPEFACE_CACHE = new HashMap();
    ...
    private static final HiLogLabel TAG = new HiLogLabel(3, LogDomain.END, "AGP");
    ...
    private static native String[] nativeGetSystemTypefaceNames();
    ...
    public static Typeface createFromFile(File file) {
        ...
        try {
            ...
            Typeface typeface = APP_TYPEFACE_CACHE.get(canonicalPath);
            if (typeface != null) {
                HiLog.info(TAG, "Typeface.createFromFile hit cache", new Object[0]);
                return typeface;
            }
            TypefaceImplWrapper typefaceImplWrapper = new TypefaceImplWrapper();
            nativeCreateFromFile(file, canonicalPath, typefaceImplWrapper);
            if (typefaceImplWrapper.nativeTypeface == 0) {
                HiLog.debug(TAG, "Font file not found: %{private}s.", canonicalPath);
                return DEFAULT;
            }
            Typeface typeface2 = new Typeface(typefaceImplWrapper.nativeTypeface, true);
            typeface2.mTypefaceImpl = typefaceImplWrapper.typefaceImpl;
            APP_TYPEFACE_CACHE.put(canonicalPath, typeface2);
            return typeface2;
        } catch (IOException unused) {
            HiLog.warn(TAG, "Invalid file path, return default font", new Object[0]);
            return DEFAULT;
        }
    }
    ...
    public static String[] getSystemTypefaceNames() {
        return nativeGetSystemTypefaceNames();
    }
