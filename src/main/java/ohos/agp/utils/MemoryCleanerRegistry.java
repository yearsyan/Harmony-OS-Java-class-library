...
public class MemoryCleanerRegistry {
    ...
    private static final HiLogLabel LOG_TAG = new HiLogLabel(3, LogDomain.END, "MemoryCleanerRegistry");
    ...
    private void runCleaner() {
        C0000Bytrace.startTrace(C0000Bytrace.BYTRACE_TAG_GRAPHIC_AGP, "MemoryCleaner");
        ...
        if (i2 > 0) {
            HiLog.debug(LOG_TAG, "Do ui memory clean, count: %{public}d, duration: %{public}l, remaining for clean: %{public}d, total remaining: %{public}d, count in callback helper: %{public}d, ref count in jni: %{public}d", Integer.valueOf(i2), Long.valueOf((j - nanoTime) / 1000), Integer.valueOf(GC_LIST.size()), Integer.valueOf(this.mCleanerMap.size()), Integer.valueOf(CallbackHelper.getCount()), Integer.valueOf(nativeGetRefCount()));
            if (this.mCleanerMap.size() > 10000) {
                HiLog.debug(LOG_TAG, "Too many agp ui objects, requiring GC", new Object[0]);
                System.gc();
            }
        }
        C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_GRAPHIC_AGP, "MemoryCleaner");
    }

    /* access modifiers changed from: private */
    public static class UIReleaseThread extends Thread {
        private static final VsyncScheduler.FrameCallback FRAME_CALLBACK = $$Lambda$MemoryCleanerRegistry$UIReleaseThread$30cZV70ltDM4y62aUAC5_67gm4M.INSTANCE;
            ...
            while (true) {
                try {
                    ...
                    if (remove instanceof WeakReference) {
                        ...
                        VsyncScheduler.getInstance().postRequestVsync(FRAME_CALLBACK);
                    }
                } catch (InterruptedException e) {
                    HiLog.error(MemoryCleanerRegistry.LOG_TAG, "UI Release Thread exception in queue: %{public}s", e.getMessage());
                }
            }
        }
    }
    ...
    private static class UncaughtExceptionLogger implements Thread.UncaughtExceptionHandler {
        ...
        public void uncaughtException(Thread thread, Throwable th) {
            HiLog.error(MemoryCleanerRegistry.LOG_TAG, "UI Release Thread exception: %{public}s", th.getMessage());
        }
    }
}
