...
public class VirtualDisplayHandler {
    private static final HiLogLabel LABEL = new HiLogLabel(3, LogDomain.END, "VirtualDisplayHandler");
    ...
    public Optional<View> getPrententationRootView() {
        ...
        if (presentationHandler != null) {
            return Optional.ofNullable(presentationHandler.getRootView());
        }
    ...
    private boolean init() {
        if (this.mContext == null || this.mSurfaceTexture == null || this.mOutView == null) {
            HiLog.error(LABEL, "invalid inputs", new Object[0]);
            return false;
        } else if (!initDisplayMetrics()) {
    ...
    private boolean isWidthHeightValid() {
        ...
        if (i <= 0 || this.mHeight <= 0) {
            ...
            HiLog.error(LABEL, "invalid size, width:%d, height:%d, metricWidth:%d, metricHeight:%d", Integer.valueOf(this.mWidth), Integer.valueOf(this.mHeight), Integer.valueOf(this.mDisplayMetrics.widthPixels), Integer.valueOf(this.mDisplayMetrics.heightPixels));
            return false;
        }
    }
}
