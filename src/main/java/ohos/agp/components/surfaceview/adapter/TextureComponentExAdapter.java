...
public class TextureComponentExAdapter {
    private static final HiLogLabel LABEL = new HiLogLabel(3, LogDomain.END, "TextureComponentExAdapter");
    ...
    public void dispatchEvent(TouchEvent touchEvent) {
        VirtualDisplayHandler virtualDisplayHandler = this.mVirtualDisplayHandler;
        if (virtualDisplayHandler == null) {
            HiLog.error(LABEL, "mVirtualDisplayHandler is null", new Object[0]);
            return;
        }
        Optional<View> prententationRootView = virtualDisplayHandler.getPrententationRootView();
        Optional<MotionEvent> hostMotionEvent = MultimodalEventFactory.getHostMotionEvent(touchEvent);
        if (!hostMotionEvent.isPresent()) {
            ...
            prententationRootView.get().dispatchTouchEvent(hostMotionEvent.get());
        }
    }
