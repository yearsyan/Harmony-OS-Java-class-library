...
public class RemoteViewUtils {
    ...
    private static final HiLogLabel LOG_TAG = new HiLogLabel(3, LogDomain.END, "RemoteViewUtils");
    ...
    private Class<?> mCls;
    private final HashMap<String, AFunctionParam> mDynamicFunctionMap;
    ...
    public RemoteViewUtils(ohos.app.Context context, int i, Class<?> cls) {
        Optional<ApplicationInfo> optional;
        this.mDynamicFunctionMap = new HashMap<>();
        this.mApplyFlags = 0;
        HiLog.debug(LOG_TAG, "RemoteViewUtils Create by context, userId and cls enter.", new Object[0]);
        if (context != null) {
            ...
            if (hostContext instanceof Context) {
                ...
                if (i == Integer.MIN_VALUE) {
                    optional = getApplicationInfo(this.mAOSPContext.getPackageName(), UserHandle.myUserId());
                } else {
                    optional = getApplicationInfo(this.mAOSPContext.getPackageName(), i);
                }
                if (optional.isPresent()) {
                    this.mApplicationInfo = optional.get();
                }
                this.mCls = cls;
            ...
            throw new IllegalArgumentException("RemoteViewUtils cannot get A Context.");
        }
        HiLog.error(LOG_TAG, "RemoteViewUtils context is null", new Object[0]);
        throw new IllegalArgumentException("RemoteViewUtils context is null");
    }
    ...
    public Optional<RemoteViews> getARemoteViews(ComponentProvider componentProvider) {
        ...
        Class<?> cls = this.mCls;
        if (cls != null) {
            HiLog.debug(LOG_TAG, "getARemoveView get layout id by given class", new Object[0]);
            this.mLayoutId = ResourceManagerInner.getAResId(this.mNativeLayoutId, this.mCls, this.mAOSPContext);
        } else {
            String str = getResourceBundleName() + ".ResourceTable";
            try {
                Class<?> cls2 = Class.forName(str, false, this.mAOSPContext.getClassLoader());
                this.mLayoutId = ResourceManagerInner.getAResId(this.mNativeLayoutId, cls2, this.mAOSPContext);
                HiLog.debug(LOG_TAG, "RemoteViewUtils mLayoutId=0x%{public}x.", Integer.valueOf(this.mLayoutId));
                cls = cls2;
            } catch (ClassNotFoundException unused) {
                HiLog.error(LOG_TAG, "RemoteViewUtils ClassNotFoundException ClassName = %{public}s", str);
                return Optional.empty();
            }
        }
        ...
        for (ComponentProvider.Action action : componentProvider.getActions()) {
            ...
            if (!privateValue.isPresent()) {
                HiLog.debug(LOG_TAG, "getARemoveView viewId not Present z viewID=0x%{public}x.", Integer.valueOf(action.viewId));
            } else {
                i = getInt(privateValue.get());
                HiLog.debug(LOG_TAG, "getARemoveView z viewID=0x%{public}x.", Integer.valueOf(i));
            }
            this.mViewId = ResourceManagerInner.getAResId(i, cls, this.mAOSPContext);
            HiLog.debug(LOG_TAG, "getARemoveView mViewId=0x%{public}x.", Integer.valueOf(this.mViewId));
    ...
    private void dealWithAction(ComponentProvider.Action action) {
        ...
        HiLog.debug(LOG_TAG, "dealWithAction mViewId=%{public}s.", name);
        switch (name.hashCode()) {
            ...
            case -762566419:
                ...
                break;
            case -394232771:
                if (name.equals("ohos.agp.components.ComponentProvider$SetImageContentAction")) {
                    c = '\t';
                    break;
                }
                c = 65535;
            ...
            case -268420068:
                ...
                break;
            case 273790229:
                if (name.equals("ohos.agp.components.ComponentProvider$SetPixelMapAction")) {
                    c = 6;
                    break;
                }
                c = 65535;
        ...
        switch (c) {
            ...
            case '\b':
                ...
                return;
            case '\t':
                doSetImageContentAction(action);
                return;
            default:
    ...
    private void doSetPixelMapAction(ComponentProvider.Action action) {
        Optional<Object> privateValue = getPrivateValue(action, "methodName");
        if (!privateValue.isPresent()) {
            HiLog.error(LOG_TAG, "doSetPixelMapAction methodName is not Present.", new Object[0]);
            return;
        }
        String string = getString(privateValue.get());
        Optional<Object> privateValue2 = getPrivateValue(action, "pixelMap");
        if (!privateValue2.isPresent()) {
            HiLog.error(LOG_TAG, "doSetPixelMapAction pixelMap is not Present.", new Object[0]);
            return;
        }
        PixelMap pixelMap = getPixelMap(privateValue2.get());
        HiLog.info(LOG_TAG, "doSetPixelMapAction pixelMap=%{public}s", pixelMap);
        if ("setPixelMap".equals(string)) {
            ...
            HiLog.error(LOG_TAG, "doSetPixelMapAction bitmap is null.", new Object[0]);
        } else if ("setBackground".equals(string)) {
            doSetBackgroundPixelMapAction(action);
        } else {
            HiLog.error(LOG_TAG, "doSetPixelMapAction error, unknown method name.", new Object[0]);
        }
    }

    private void doSetImageContentAction(ComponentProvider.Action action) {
        Optional<Object> privateValue = getPrivateValue(action, "resId");
        if (!privateValue.isPresent()) {
            HiLog.error(LOG_TAG, "doSetImageContentAction resId not Present.", new Object[0]);
            return;
        }
        this.mARemoteView.setInt(this.mViewId, "setImageResource", convertResourceId(getInt(privateValue.get())));
    }
    ...
    private void doDynamicAction(ComponentProvider.Action action) {
        ...
        Optional<Object> privateValue2 = getPrivateValue(action, "method");
        if (!privateValue2.isPresent()) {
        ...
        HiLog.debug(LOG_TAG, "dealWithAction actionType=%{public}d, method=%{public}s.", Integer.valueOf(i), string);
        dynamicFunctionMap(i, string, obj);
    }

    private void dynamicFunctionMap(int i, String str, Object obj) {
        AFunctionParam aFunctionParam = this.mDynamicFunctionMap.get(str);
    ...
    private int convertIntValueIfNecessary(String str, int i) {
        if (!"setVisibility".equals(str)) {
            return ("setImageResource".equals(str) || "setIgnoreGravity".equals(str)) ? convertResourceId(i) : i;
        }
        if (i == 0) {
            return 0;
        }
    ...
    private void doViewGroupLayoutParamAction(ComponentProvider.Action action) {
        ...
        HiLog.debug(LOG_TAG, "doViewGroupLayoutParamAction width=%{public}d.", Integer.valueOf(i));
    ...
    private void doSetTextColorAction(ComponentProvider.Action action) {
        ...
        int i = getInt(privateValue.get());
        HiLog.debug(LOG_TAG, "doSetTextColorAction color=%{public}d.", Integer.valueOf(i));
    ...
    private void doSetTextSizeAction(ComponentProvider.Action action) {
        ...
        HiLog.debug(LOG_TAG, "doSetTextSizeAction size=%{public}d textSizeType=%{public}d.", Integer.valueOf(i), Integer.valueOf(i2));
    ...
    private void doViewGroupMarginLayoutAction(ComponentProvider.Action action) {
        ...
        HiLog.debug(LOG_TAG, "doViewGroupMarginLayoutAction topMargin=%{public}d.", Integer.valueOf(i));
    ...
    private void doSetProgressBarAction(ComponentProvider.Action action) {
        ...
        HiLog.debug(LOG_TAG, "doSetProgressBarAction progress=%{public}d.", Integer.valueOf(i2));
    ...
    private void doSetPaddingAction(ComponentProvider.Action action) {
        ...
        HiLog.debug(LOG_TAG, "doSetProgressBarAction top=%{public}d.", Integer.valueOf(i2));
    ...
    private void doSetBackgroundPixelMapAction(ComponentProvider.Action action) {
        ...
        View findViewById;
        Optional<Object> privateValue = getPrivateValue(action, "pixelMap");
        if (!privateValue.isPresent()) {
    ...
    private PixelMap getPixelMap(Object obj) {
        if (obj instanceof PixelMap) {
            return (PixelMap) obj;
        }
        HiLog.error(LOG_TAG, "getPixelMap obj not PixelMap.", new Object[0]);
        return null;
    }
    ...
    private void marshallApplicationInfo(Parcel parcel) {
        if (this.mApplicationInfo == null) {
            HiLog.error(LOG_TAG, "marshallApplicationInfo error current mApplicationInfo is null.", new Object[0]);
            return;
        }
        android.os.Parcel obtain = android.os.Parcel.obtain();
        try {
            ...
            obtain.setDataPosition(0);
            parcel.writeBytes(obtain.marshall());
        } finally {
            obtain.recycle();
        }
    }
    ...
    private void marshallLayoutId(int i, Parcel parcel) {
        ...
        try {
            if (this.mCls != null) {
                HiLog.debug(LOG_TAG, "marshallLayoutId get layout id by given class", new Object[0]);
                i2 = ResourceManagerInner.getAResId(i, this.mCls, this.mAOSPContext);
            } else {
                i2 = ResourceManagerInner.getAResId(i, Class.forName(str, false, this.mAOSPContext.getClassLoader()), this.mAOSPContext);
            }
            parcel.writeInt(i2);
            HiLog.debug(LOG_TAG, "remote layoutId=0x%{public}x.", Integer.valueOf(i2));
        } catch (ClassNotFoundException unused) {
            HiLog.error(LOG_TAG, "ClassNotFoundException ClassName=%{public}s", str);
        }
    }
    ...
    private int getRemoteViewId(ComponentProvider.Action action) {
        ...
        Optional<Object> privateValue = getPrivateValue(action, "viewId");
        if (!privateValue.isPresent()) {
            HiLog.debug(LOG_TAG, "viewId not Present, z viewId=0x%{public}x.", Integer.valueOf(action.viewId));
        } else {
            i = getInt(privateValue.get());
            HiLog.debug(LOG_TAG, "z viewId=0x%{public}x.", Integer.valueOf(i));
        }
    ...
    public int convertResourceId(int i) {
        ...
        try {
            ...
            try {
                HiLog.debug(LOG_TAG, "remote resourceId=0x%{public}x.", Integer.valueOf(i));
            } catch (ClassNotFoundException unused) {
            }
    ...
    private void traverseAction(ComponentProvider.Action action) {
        ...
        String name = action.getClass().getName();
        HiLog.debug(LOG_TAG, "actionName=%{public}s.", name);
        switch (name.hashCode()) {
            case -1030354205:
                if (name.equals("ohos.agp.components.ComponentProvider$SetTextColorAction")) {
                    c = 1;
        ...
        switch (c) {
            ...
            case 5:
                ...
                return;
            case 6:
                marshallSetImageContentAction(action);
                return;
            case 7:
                marshallSetPixelMapAction(action);
                return;
            case '\b':
                return;
            default:
    ...
    private void marshallSetPixelMapAction(ComponentProvider.Action action) {
        Optional<Object> privateValue = getPrivateValue(action, "methodName");
        if (!privateValue.isPresent()) {
            HiLog.error(LOG_TAG, "marshallSetPixelMapAction methodName is not Present.", new Object[0]);
            return;
        }
        String string = getString(privateValue.get());
        Optional<Object> privateValue2 = getPrivateValue(action, "pixelMap");
        if (!privateValue2.isPresent()) {
            HiLog.error(LOG_TAG, "marshallSetPixelMapAction pixelMap is not Present.", new Object[0]);
            return;
        }
        PixelMap pixelMap = getPixelMap(privateValue2.get());
        int remoteViewId = getRemoteViewId(action);
        if ("setPixelMap".equals(string)) {
            addAction(new PixelMapReflectionAction(remoteViewId, "setImageBitmap", pixelMap));
        } else {
            HiLog.error(LOG_TAG, "marshallSetPixelMapAction error, method name is not equal to setPixelMap", new Object[0]);
        }
    }

    private void marshallSetImageContentAction(ComponentProvider.Action action) {
        Optional<Object> privateValue = getPrivateValue(action, "resId");
        if (!privateValue.isPresent()) {
            HiLog.error(LOG_TAG, "marshallSetImageContentAction resId not Present.", new Object[0]);
            return;
        }
        addAction(new ReflectionAction(getRemoteViewId(action), "setImageResource", 4, Integer.valueOf(convertResourceId(getInt(privateValue.get())))));
    }
    ...
    private void marshallDynamicAction(ComponentProvider.Action action) {
        ...
        Object obj = privateValue3.get();
        AFunctionParam aFunctionParam = this.mDynamicFunctionMap.get(string);
        if (aFunctionParam == null) {
            HiLog.error(LOG_TAG, "method=%{public}s not found in DynamicFunctionMap.", string);
            return;
        }
        int remoteViewId = getRemoteViewId(action);
    ...
    private void finishDynamicAction(int i, int i2, String str, Object obj) {
        HiLog.debug(LOG_TAG, "finishDynamicAction: actionType=%{public}d, method=%{public}s.", Integer.valueOf(i2), str);
        if (i2 == 1) {
    ...
    private void marshallSetTextColorAction(ComponentProvider.Action action) {
        ...
        int i = getInt(privateValue.get());
        HiLog.debug(LOG_TAG, "color=0x%{public}x.", Integer.valueOf(i));
    ...
    private void marshallSetPaddingAction(ComponentProvider.Action action) {
        ...
        int i4 = getInt(privateValue4.get());
        HiLog.debug(LOG_TAG, "left=%{public}d top=%{public}d right=%{public}d bottom=%{public}d.", Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(i4));
    ...
    private void marshallSetProgressBarAction(ComponentProvider.Action action) {
        ...
        boolean z = getBoolean(privateValue3.get());
        HiLog.debug(LOG_TAG, "max=%{public}d progress=%{public}d indeterminate=%{public}d.", Integer.valueOf(i), Integer.valueOf(i2), Boolean.valueOf(z));
        int remoteViewId = getRemoteViewId(action);
    ...
    private void marshallComponentContainerLayoutAction(ComponentProvider.Action action) {
        ...
        int i = getInt(privateValue.get());
        if (i == 0 || i == -2 || i == -1) {
            HiLog.debug(LOG_TAG, "layoutWidth=%{public}d.", Integer.valueOf(i));
            ...
            int i4 = getInt(privateValue4.get());
            HiLog.debug(LOG_TAG, "topMargin=%{public}d bottomMargin=%{public}d rightMargin=%{public}d.", Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(i4));
            int remoteViewId = getRemoteViewId(action);
    ...
    public static class PixelMapCache {
        ...
        public void writePixelMapsToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "PixelMapCache writePixelMapsToParcel dest is null", new Object[0]);
                return;
            }
    ...
    public class PixelMapReflectionAction implements Action {
        ...
        public void writeToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "PixelMapReflectionAction writeToParcel dest is null", new Object[0]);
                return;
            }
    ...
    public static class ViewPaddingAction implements Action {
        ...
        public void writeToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "ViewPaddingAction writeToParcel dest is null", new Object[0]);
                return;
            }
            parcel.writeInt(this.mViewId);
    ...
    public static class LayoutParamAction implements Action {
        ...
        public void writeToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "LayoutParamAction writeToParcel dest is null", new Object[0]);
                return;
            }
            parcel.writeInt(this.mViewId);
    ...
    private static class TextViewSizeAction implements Action {
        ...
        public void writeToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "TextViewSizeAction writeToParcel dest is null", new Object[0]);
                return;
            }
            parcel.writeInt(this.mViewId);
    ...
    public final class ReflectionAction implements Action {
        ...
        public void writeToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "ReflectionAction writeToParcel dest is null", new Object[0]);
                return;
            }
            parcel.writeInt(this.mViewId);
            parcel.writeString(this.mMethodName);
    ...
    public static class SetOnClickResponse implements Action {
        ...
        public void writeToParcel(Parcel parcel) {
            if (parcel == null) {
                HiLog.error(RemoteViewUtils.LOG_TAG, "SetOnClickResponse writeToParcel dest is null", new Object[0]);
                return;
            }
            parcel.writeInt(this.mViewId);
