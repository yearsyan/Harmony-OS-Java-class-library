...
public class AnimatorProperty extends Animator implements Component.AvailabilityObserver {
    private static final HiLogLabel TAG = new HiLogLabel(3, LogDomain.END, "AGP_ViewPropertyAnimator");
    ...
    public AnimatorProperty(Component component) {
        ...
        this.floatAttr = "alpha,alphaBy,alphaFrom,moveFromX,moveToX,moveFromY,moveToY,moveByX,moveByY,scaleX,scaleXFrom,scaleXBy,scaleY,scaleYFrom,scaleYBy,rotation,rotationBy,rotationX,rotationXBy,rotationY,rotationYBy";
        this.floatSet = new HashSet<>(Arrays.asList("alpha,alphaBy,alphaFrom,moveFromX,moveToX,moveFromY,moveToY,moveByX,moveByY,scaleX,scaleXFrom,scaleXBy,scaleY,scaleYFrom,scaleYBy,rotation,rotationBy,rotationX,rotationXBy,rotationY,rotationYBy".split(",")));
        ...
        for (TypedAttribute typedAttribute : node.getTypedAttributes(resourceManager)) {
            if (typedAttribute == null) {
                ...
                if (name == null) {
                    ...
                    HiLog.debug(TAG, "read viewPropertyAnimator attr: %{public}s", name);
    ...
    private void setPropertyValue(String str, TypedAttribute typedAttribute) {
        try {
            if (this.intSet.contains(str)) {
                HiLog.debug(TAG, "apply viewPropertyAnimator attr: %{public}s , value: %{public}d", str, Integer.valueOf(typedAttribute.getIntegerValue()));
                char c = 65535;
                switch (str.hashCode()) {
                    case -2032139925:
                ...
                try {
                    ...
                    HiLog.debug(TAG, "method is %{public}s", declaredMethod);
                    declaredMethod.setAccessible(true);
                    HiLog.debug(TAG, "apply viewPropertyAnimator attr: %{public}s, value: %{public}f", str, Float.valueOf(convertFloat(typedAttribute.getStringValue())));
                ...
                HiLog.error(TAG, "do not support this tag: %{public}s", str);
            }
    ...
    public AnimatorProperty alphaFrom(float f) {
        nativeAlphaFrom(this.mNativeAnimatorPtr, f);
        return this;
    }
    ...
    public AnimatorProperty scaleXFrom(float f) {
        nativeScaleXFrom(this.mNativeAnimatorPtr, f);
        return this;
    }
    ...
    public AnimatorProperty scaleYFrom(float f) {
        nativeScaleYFrom(this.mNativeAnimatorPtr, f);
        return this;
    }
