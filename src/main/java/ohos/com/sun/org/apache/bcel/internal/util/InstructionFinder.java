...
public class InstructionFinder {
    ...
    public final Iterator search(String str, InstructionHandle instructionHandle, CodeConstraint codeConstraint) {
        ...
        if (i != -1) {
            Pattern compile = Pattern.compile(compilePattern);
            ArrayList arrayList = new ArrayList();
            Matcher matcher = compile.matcher(this.il_string);
            while (i < this.il_string.length() && matcher.find(i)) {
                int start = matcher.start();
                int end = matcher.end();
                InstructionHandle[] match = getMatch(start, end - start);
