...
public class DOMNormalizer implements XMLDocumentHandler {
    ...
    /* JADX DEBUG: Failed to insert an additional move for type inference into block B:192:0x01dd */
    /* access modifiers changed from: protected */
    public Node normalizeNode(Node node) {
        ...
        if (nodeType == 1) {
                    ...
                    if (nodeType == 8) {
                        if ((this.fConfiguration.features & 32) == 0) {
                            ...
                            if (previousSibling != null && previousSibling.getNodeType() == 3 && (nextSibling = previousSibling.getNextSibling()) != null && nextSibling.getNodeType() == 3) {
                                ((TextImpl) nextSibling).insertData(0, previousSibling.getNodeValue());
            ...
            if (previousSibling3 == null || previousSibling3.getNodeType() != 3) {
                Node createTextNode = this.fDocument.createTextNode(node.getNodeValue());
            ...
            if ((this.fConfiguration.features & 16) != 0) {
                ...
                while (true) {
                    ...
                    Node createCDATASection = this.fDocument.createCDATASection(nodeValue);
    ...
    private XMLGrammarPool createGrammarPool(DocumentTypeImpl documentTypeImpl) {
        ...
        try {
            ((XMLDTDDescription) ((DTDGrammar) xMLGrammarPreparser.preparseGrammar("http://www.w3.org/TR/REC-xml", xMLInputSource)).getGrammarDescription()).setRootName(documentTypeImpl.getName());
                    ...
                    if (namespaceURI3.equals(NamespaceContext.XMLNS_URI)) {
                        ...
                        if (!this.fDocument.errorChecking || (this.fConfiguration.features & 256) == 0) {
                            ...
                            if (this.fDocument.isXMLVersionChanged() && !CoreDocumentImpl.isXMLName(attr2.getNodeName(), this.fDocument.isXML11Version())) {
                                reportDOMError(this.fErrorHandler, this.fError, this.fLocator, DOMMessageFormatter.formatMessage(DOMMessageFormatter.DOM_DOMAIN, "wf-invalid-character-in-node-name", new Object[]{"Attribute", attr2.getNodeName()}), 2, "wf-invalid-character-in-node-name");
                            }
                        }
