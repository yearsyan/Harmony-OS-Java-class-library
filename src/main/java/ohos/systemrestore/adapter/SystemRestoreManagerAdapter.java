...
public class SystemRestoreManagerAdapter {
    ...
    private SystemRestoreManagerAdapter() {
        SystemRestoreStringUtil.printDebug(TAG, "SystemRestoreManagerAdapter Construction.");
    }
        ...
        synchronized (SystemRestoreManagerAdapter.class) {
            if (adapterInstance == null) {
                try {
                    ...
                    SystemRestoreStringUtil.printException(TAG, e, "SystemRestoreManagerAdapter not found recoveryService.", false);
                }
                try {
                    ...
                    SystemRestoreStringUtil.printException(TAG, e2, "SystemRestoreManagerAdapter not found user manager service.", false);
                }
    ...
    private boolean rebootRestorePartition(String str) throws SystemRestoreException {
        if (recoveryService != null) {
            SystemRestoreStringUtil.printDebug(TAG, "rebootRestorePartition before exec in adapter");
            if (!SystemRestoreStringUtil.isEmpty(str)) {
                ...
                HiLog.error(TAG, "rebootRestorePartition exec  command is empty.", new Object[0]);
    ...
    private boolean passedAuthenticationRestore(Context context) throws SystemRestoreException {
        ...
        try {
            ...
            SystemRestoreStringUtil.printDebug(TAG, "reboot restore passed authentication.");
            return true;
        } catch (SecurityException e) {
            SystemRestoreStringUtil.printException(TAG, e, "enforceCallingOrSelfPermission exception");
            throw new SystemRestoreException(e.getMessage());
        }
    }
    ...
    public void sendRestoreUserDataBroadcast(Context context) throws SystemRestoreException {
        SystemRestoreStringUtil.printDebug(TAG, "sendRestoreUserDataBroadcast start.");
        android.content.Context adapterContext = getAdapterContext(context);
        ConditionVariable conditionVariable = new ConditionVariable();
        Intent intent = new Intent("android.intent.action.MASTER_CLEAR_NOTIFICATION");
        intent.addFlags(285212672);
        SystemRestoreStringUtil.printDebug(TAG, "sendRestoreUserDataBroadcast before send.");
        ...
        SystemRestoreStringUtil.printDebug(TAG, "sendRestoreUserDataBroadcast wait block.");
        conditionVariable.block(1000);
        SystemRestoreStringUtil.printDebug(TAG, "sendRestoreUserDataBroadcast end.");
    }
    ...
    private static class SystemRestoreBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(android.content.Context context, Intent intent) {
            SystemRestoreStringUtil.printDebug(SystemRestoreManagerAdapter.TAG, "sendRestoreUserDataBroadcast receive.");
            this.condition.open();
        }
    }
}
