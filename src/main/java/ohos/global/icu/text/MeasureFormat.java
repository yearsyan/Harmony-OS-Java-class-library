    ...
    private void formatMeasuresInternal(Appendable appendable, FieldPosition fieldPosition, Measure... measureArr) {
        ...
        if (measureArr.length != 0) {
            if (measureArr.length == 1) {
                ...
                instance.formatImpl(Arrays.asList(strArr), false).appendTo(appendable);
            } else {
    ...
    private void formatMeasuresSlowTrack(ListFormatter listFormatter, Appendable appendable, FieldPosition fieldPosition, Measure... measureArr) {
        ...
        for (int i2 = 0; i2 < measureArr.length; i2++) {
            if (i2 == measureArr.length - 1) {
                formattedStringBuilder = formatMeasure(measureArr[i2]);
            } else {
                formattedStringBuilder = formatMeasureInteger(measureArr[i2]);
            }
            if (i == -1) {
            ...
            strArr[i2] = formattedStringBuilder.toString();
        }
        ...
        while (i2 < str.length()) {
            ...
            if (charAt == '\'') {
                ...
                if (z) {
                    ...
                    i = i2 + 1;
                    if (i >= str.length() || str.charAt(i) != charAt) {
                        formattedStringBuilder.append(this.numberFormatter.format(num), (Object) null);
                    } else {
                        formattedStringBuilder.append(localizedNumberFormatter.format(num), (Object) null);
                    }
                }
                i2++;
                c = 0;
    ...
    public static String getRangeFormat(ULocale uLocale, FormatWidth formatWidth2) {
        ...
        if (str3 == null) {
            ...
            if (uLocale.equals(uLocale2) || (str2 = localeIdToRangeFormat.get(uLocale)) == null) {
                ...
                try {
                    str = iCUResourceBundle.getStringWithFallback("NumberElements/" + instance.getName() + "/miscPatterns/range");
                } catch (MissingResourceException unused) {
                    str = iCUResourceBundle.getStringWithFallback("NumberElements/latn/patterns/range");
                }
