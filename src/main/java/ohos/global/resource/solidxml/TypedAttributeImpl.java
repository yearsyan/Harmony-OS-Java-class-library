...
public class TypedAttributeImpl extends TypedAttribute {
    private static Pattern COLOR_PATTERN = Pattern.compile("^#(?<color>([A-F0-9]{6}|[A-F0-9]{8}))$");
    ...
    public int getColorValue() throws NotExistException, IOException, WrongTypeException {
        ...
        if (str == null || str.length() == 0 || this.resManager == null) {
            ...
            Matcher matcher = COLOR_PATTERN.matcher(this.attrValue.toUpperCase());
            if (matcher.find()) {
                String group = matcher.group("color");
                int parseUnsignedInt = Integer.parseUnsignedInt(group, 16);
                if (group.length() == 6) {
                    return parseUnsignedInt | -16777216;
                }
    ...
    @Override // ohos.global.resource.solidxml.TypedAttribute
    public Resource getMediaResource() throws NotExistException, IOException, WrongTypeException {
        String str = this.attrValue;
        if (str == null || str.length() == 0 || this.resManager == null) {
            throw new NotExistException("value not exist");
        }
        return this.resManager.getRawFileEntry(getMediaValue()).openRawFile();
    }

    @Override // ohos.global.resource.solidxml.TypedAttribute
    public int getPixelValue(boolean z) throws NotExistException, IOException, WrongTypeException {
        String str = this.attrValue;
        if (str == null || str.length() == 0 || this.resManager == null) {
            throw new NotExistException("value not exist");
        }
        String str2 = this.attrValue;
        if (str2.startsWith("@") || this.attrValue.startsWith("$")) {
            str2 = this.resManager.getElement(tryDereferrence(this.attrValue)).getString();
        }
        float f = 0.0f;
        ResourceManager resourceManager = this.resManager;
        if (resourceManager instanceof ResourceManagerImpl) {
            f = ((ResourceManagerImpl) resourceManager).parseFloat(str2);
        }
        if (z) {
            return (int) f;
        }
    ...
    @Override // ohos.global.resource.solidxml.TypedAttribute
    public String getParsedValue() throws NotExistException, IOException, WrongTypeException {
        String str = this.attrValue;
        if (str == null || str.length() == 0 || this.resManager == null) {
            throw new NotExistException("value not exist");
        }
