...
public final class ContinuationManager {
    ...
    private boolean doScheduleStartContinuation() {
        ...
        C0000Bytrace.startTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onStartContinuation");
        if (!this.continuableAbility.onStartContinuation()) {
            C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onStartContinuation");
            return false;
        }
        C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onStartContinuation");
    ...
    private boolean doScheduleSaveData(IntentParams intentParams) {
        if (intentParams == null) {
            ...
            C0000Bytrace.startTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onSaveData");
            boolean onSaveData = this.continuableAbility.onSaveData(intentParams2);
            C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onSaveData");
    ...
    private boolean doScheduleRestoreData(IntentParams intentParams) {
        if (intentParams == null) {
            ...
            C0000Bytrace.startTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onRestoreData");
            if (!this.continuableAbility.onRestoreData(intentParams2)) {
                Log.warn(LABEL, "Ability restore data failed.", new Object[0]);
                C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onRestoreData");
                return false;
            }
            C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onRestoreData");
    ...
    public void completeContinuation(int i) {
        ...
        changeProcessState(ProgressState.INITIAL);
        C0000Bytrace.startTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onCompleteContinuation");
        this.continuableAbility.onCompleteContinuation(i);
        C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onCompleteContinuation");
    ...
    public void notifyRemoteTerminated() {
        ...
        this.continuationState = ContinuationState.LOCAL_RUNNING;
        changeProcessState(ProgressState.INITIAL);
        C0000Bytrace.startTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onRemoteTerminated");
        this.continuableAbility.onRemoteTerminated();
        C0000Bytrace.finishTrace(C0000Bytrace.BYTRACE_TAG_ABILITY_MANAGER, "onRemoteTerminated");
    ...
    private static void debugLog(AbilityInfo abilityInfo, long j) {
        if (abilityInfo != null) {
            long nanoTime = System.nanoTime();
            JLog.debug(JLogConstants.JLID_LOCAL_CONTINUE_ABILITY_BEGIN, abilityInfo.getBundleName() + "/" + abilityInfo.getClassName() + " cost: " + ((nanoTime - j) / 1000000) + DateFormat.MINUTE_SECOND);
        }
    }
}
