...
class DistributedInitCallbackDelegate extends RemoteObject implements IRemoteBroker, IInitCallback {
    ...
    public boolean onRemoteRequest(int i, MessageParcel messageParcel, MessageParcel messageParcel2, MessageOption messageOption) throws RemoteException {
        ...
        HiLog.debug(TAG, "DmsDistributedCallbackDelegate onRemoteRequest code is %{private}d", Integer.valueOf(i));
        String readInterfaceToken = messageParcel.readInterfaceToken();
        if (!DESCRIPTOR.equals(readInterfaceToken)) {
            HiLog.error(TAG, "onRemoteRequest error interfaceToken:%{private}s", readInterfaceToken);
            return false;
        }
        if (i == 1) {
