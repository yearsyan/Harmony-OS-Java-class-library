...
public class NdefTag extends TagManager {
    ...
    public NdefTag(TagInfo tagInfo) {
        ...
        if (tagInfo != null) {
            if (tagInfo.isProfileSupported(6)) {
                PacMapEx orElse = tagInfo.getProfileExtras(6).orElse(null);
                if (orElse != null) {
                    this.mMaxNdefSize = ((Integer) orElse.getObjectValue(EXTRA_NDEF_MAXLENGTH).get()).intValue();
                    this.mCardState = ((Integer) orElse.getObjectValue(EXTRA_NDEF_CARDSTATE).get()).intValue();
                    if (orElse.getObjectValue(EXTRA_NDEF_MSG).get() instanceof NdefMessage) {
                        this.mNdefMsg = (NdefMessage) orElse.getObjectValue(EXTRA_NDEF_MSG).get();
                    } else {
                        this.mNdefMsg = new NdefMessage(new MessageRecord[0]);
                    }
                    this.mNdefType = ((Integer) orElse.getObjectValue(EXTRA_NDEF_TYPE).get()).intValue();
                } else {
