...
public class MifareUltralightTag extends TagManager {
    ...
    private MifareUltralightTag(TagInfo tagInfo) {
        ...
        short s = -1;
        if ((instance != null ? instance.getSak() : s) == 0 && tagInfo.getTagId()[0] == 4 && (orElse = tagInfo.getProfileExtras(9).orElse(null)) != null && (orElse.getObjectValue(EXTRA_IS_ULC).get() instanceof Boolean)) {
            if (((Boolean) orElse.getObjectValue(EXTRA_IS_ULC).get()).booleanValue()) {
                this.mMifareType = 2;
            } else {
                this.mMifareType = 1;
            }
