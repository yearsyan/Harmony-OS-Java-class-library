...
public final class GeoConvert {
    ...
    public List<GeoAddress> getAddressFromLocation(double d, double d2, int i) throws IOException {
        if (d < -90.0d || d > 90.0d) {
            throw new IllegalArgumentException("latitude is illegal, should between -90.0 to 90.0");
    ...
    public List<GeoAddress> getAddressFromLocationName(String str, int i) throws IOException {
        if (str != null) {
            ArrayList arrayList = new ArrayList();
            String addressFromLocationName = this.geoConvertAdapter.getAddressFromLocationName(str, i, this.mlocale, arrayList, XPath.MATCH_SCORE_QNAME, XPath.MATCH_SCORE_QNAME, XPath.MATCH_SCORE_QNAME, XPath.MATCH_SCORE_QNAME);
