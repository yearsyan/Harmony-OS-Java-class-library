...
public class EventHandler {
    private static final HiLogLabel LOG_LABEL = new HiLogLabel(3, ICourierEx.DOMAIN, "JavaEventHandler");
    ...
    private void sendEvent(InnerEvent innerEvent, long j, Priority priority, EventType eventType) throws IllegalArgumentException {
        ...
        innerEvent.sendTime = fromNanoSecondsToMills;
        if (j > 0) {
            innerEvent.handleTime = fromNanoSecondsToMills + j;
        } else {
            innerEvent.handleTime = fromNanoSecondsToMills;
        }
        innerEvent.currentTime = System.nanoTime();
    ...
    public String toString() {
        ...
        if (this.courier != null) {
            sb.append(" courier={");
            sb.append(this.courier.toString());
            sb.append("}");
        }
        EventRunner eventRunner2 = this.eventRunner;
        if (!(eventRunner2 == null || eventRunner2.eventQueue == null)) {
            sb.append(" eventQueue={");
            sb.append(this.eventRunner.eventQueue.toString());
            sb.append("}");
        }
        sb.append("}");
        return sb.toString();
