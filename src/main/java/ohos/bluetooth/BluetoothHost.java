...
public final class BluetoothHost {
    ...
    private static final HiLogLabel TAG = new HiLogLabel(3, LogHelper.BT_DOMAIN_ID, "BluetoothHost");
    ...
    public int getProfileConnState(int i) {
        HiLog.info(TAG, "getProfileConnState : %{public}d", Integer.valueOf(i));
        if (getBtState() != 2) {
    ...
    public static boolean isValidBluetoothAddr(String str) {
        if (str == null || str.length() != 17) {
            ...
            HiLog.info(TAG, "isValidBluetoothAddr data wrong, input: %{private}s", str);
            return false;
        }
    }
}
