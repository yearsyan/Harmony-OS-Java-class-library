...
public final class PbapClient implements ProfileBase {
    private static final HiLogLabel TAG = new HiLogLabel(3, LogHelper.BT_DOMAIN_ID, "PbapClient");
    ...
    public boolean disconnect(BluetoothRemoteDevice bluetoothRemoteDevice) {
        HiLog.info(TAG, "client disconnect", new Object[0]);
        if (bluetoothRemoteDevice == null) {
            HiLog.info(TAG, "client disconnect got null device", new Object[0]);
            return false;
        } else if (this.mBluetoothHost.getBtState() == 2) {
            return this.mPbapClientProxy.disconnect(bluetoothRemoteDevice);
        } else {
            HiLog.info(TAG, "client disconnect when state not on", new Object[0]);
            return false;
        }
    }
    ...
    public boolean setConnectStrategy(BluetoothRemoteDevice bluetoothRemoteDevice, int i) {
        ...
        if (bluetoothRemoteDevice == null) {
            ...
            HiLog.info(TAG, "PbapClient setConnectStrategy got illegal strategy", new Object[0]);
            return false;
        } else if (this.mBluetoothHost.getBtState() == 2) {
