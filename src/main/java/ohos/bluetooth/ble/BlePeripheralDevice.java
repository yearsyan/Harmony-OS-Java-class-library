...
public class BlePeripheralDevice {
    ...
    private static final HiLogLabel TAG = new HiLogLabel(3, LogHelper.BT_DOMAIN_ID, "BlePeripheralDevice");
    ...
    public static BlePeripheralDevice createInstance(String str) {
        return new BlePeripheralDevice(str);
    }
    ...
    /* access modifiers changed from: package-private */
    public void updateServiceAndFixIncludeRelation(List<GattService> list) {
        ...
        for (GattService gattService : this.mServices) {
            ...
            while (it.hasNext()) {
                ...
                getService(gattService2.getUuid(), gattService2.getHandle()).ifPresent(new Consumer() {
                    /* class ohos.bluetooth.ble.$$Lambda$BlePeripheralDevice$JD9vvnmycTzhazwk6ABtOgriaZA */
    ...
    public Optional<GattService> getService(UUID uuid) {
        for (GattService gattService : this.mServices) {
            if (Objects.equals(gattService.getUuid(), uuid)) {
                return Optional.of(gattService);
            }
        }
        return Optional.empty();
    }
    ...
    public boolean readCharacteristic(GattCharacteristic gattCharacteristic) {
        if (gattCharacteristic == null || gattCharacteristic.getService() == null) {
            HiLog.error(TAG, "readCharacteristic service illegal.", new Object[0]);
            return false;
        } else if ((gattCharacteristic.getProperties() & 2) == 0) {
            HiLog.error(TAG, "readCharacteristic property illegal.", new Object[0]);
            return false;
        } else if (this.mClientHandle == 0) {
            HiLog.error(TAG, "readCharacteristic mClientHandle illegal.", new Object[0]);
            return false;
        } else {
            synchronized (this.mWriteReadLock) {
                if (this.mIsWriteReadBusy) {
                    HiLog.error(TAG, "readCharacteristic state illegal.", new Object[0]);
                    return false;
                }
    ...
    public boolean writeCharacteristic(GattCharacteristic gattCharacteristic) {
        if ((gattCharacteristic.getProperties() & 8) == 0 && (gattCharacteristic.getProperties() & 4) == 0) {
            HiLog.error(TAG, "writeCharacteristic property illegal.", new Object[0]);
            return false;
        } else if (this.mClientHandle == 0) {
            HiLog.error(TAG, "writeCharacteristic mClientHandle illegal.", new Object[0]);
            return false;
        } else if (gattCharacteristic.getValue().length == 0) {
            HiLog.error(TAG, "writeCharacteristic value illegal.", new Object[0]);
            return false;
        } else if (gattCharacteristic.getService() == null) {
            HiLog.error(TAG, "writeCharacteristic service illegal.", new Object[0]);
            return false;
        } else {
            synchronized (this.mWriteReadLock) {
                if (this.mIsWriteReadBusy) {
                    HiLog.error(TAG, "writeCharacteristic state illegal.", new Object[0]);
                    return false;
                }
                this.mIsWriteReadBusy = true;
                try {
    ...
    public boolean readDescriptor(GattDescriptor gattDescriptor) {
        if (this.mClientHandle == 0) {
            HiLog.error(TAG, "readDescriptor mClientHandle illegal.", new Object[0]);
            return false;
        }
        GattCharacteristic characteristic = gattDescriptor.getCharacteristic();
        if (characteristic == null || characteristic.getService() == null) {
            HiLog.error(TAG, "readDescriptor characteristic or service illegal.", new Object[0]);
            return false;
        }
        synchronized (this.mWriteReadLock) {
            if (this.mIsWriteReadBusy) {
    ...
    public boolean writeDescriptor(GattDescriptor gattDescriptor) {
        if (this.mClientHandle == 0) {
            HiLog.error(TAG, "writeDescriptor mClientHandle illegal.", new Object[0]);
            return false;
        } else if (gattDescriptor.getValue().length == 0) {
            HiLog.error(TAG, "writeDescriptor value illegal.", new Object[0]);
            return false;
        } else {
            GattCharacteristic characteristic = gattDescriptor.getCharacteristic();
            if (characteristic == null || characteristic.getService() == null) {
                HiLog.error(TAG, "writeDescriptor characteristic or service illegal.", new Object[0]);
                return false;
            }
            synchronized (this.mWriteReadLock) {
                if (this.mIsWriteReadBusy) {
    ...
    public boolean readRemoteRssiValue() {
        int i = this.mClientHandle;
        if (i == 0) {
            HiLog.error(TAG, "readRemoteRssiValue mClientHandle illegal.", new Object[0]);
            return false;
        }
        try {
    ...
    public boolean requestBleMtuSize(int i) {
        int i2 = this.mClientHandle;
        if (i2 == 0) {
            HiLog.error(TAG, "requestBleMtuSize mClientHandle illegal.", new Object[0]);
            return false;
        }
        try {
    ...
    public Optional<String> getDeviceName() {
        BluetoothHostProxy instace = BluetoothHostProxy.getInstace();
        if (instace == null) {
            return Optional.empty();
        }
        return instace.getRemoteName(this);
    }
}
