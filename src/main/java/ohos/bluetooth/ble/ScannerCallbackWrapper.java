...
public class ScannerCallbackWrapper extends RemoteObject implements IBleCallback {
    ...
    private static final HiLogLabel TAG = new HiLogLabel(3, LogHelper.BT_DOMAIN_ID, "ScannerCallbackWrapper");
    ...
    public boolean onRemoteRequest(int i, MessageParcel messageParcel, MessageParcel messageParcel2, MessageOption messageOption) throws RemoteException {
        boolean z = false;
        HiLog.info(TAG, "onRemoteRequest code %{public}d", Integer.valueOf(i));
        if (this.mBleCentralManager == null) {
            ...
            for (int i2 = 0; i2 < readInt3; i2++) {
                if (messageParcel.getReadableBytes() < 4) {
    ...
    private void parseRecord(RecordSet recordSet, Map<Integer, byte[]> map, Map<UUID, byte[]> map2, List<UUID> list) {
        ...
        if (i != 22) {
            if (i == 255) {
                int i4 = i3 + 1;
                if (i4 >= bArr.length || i2 <= 2) {
                    HiLog.error(TAG, "parseRecord got manufacture data fail for data length error", new Object[0]);
                    return;
                }
    ...
    private Optional<BleScanResult> readFromBytes(byte[] bArr, BlePeripheralDevice blePeripheralDevice) {
        ...
        while (true) {
            int i4 = i2 + 1;
            if (i4 >= bArr.length || (i = bArr[i2] & 255) <= 1) {
                Optional<BleScanResult> ofNullable = Optional.ofNullable(new BleScanResult(blePeripheralDevice, hashMap, hashMap2, arrayList, i3, -1));
            } else {
                int i5 = i - 1;
                int i6 = i4 + 1;
                int i7 = bArr[i4] & 255;
        ...
        Optional<BleScanResult> ofNullable2 = Optional.ofNullable(new BleScanResult(blePeripheralDevice, hashMap, hashMap2, arrayList, i3, -1));
        if (ofNullable2.isPresent()) {
            ofNullable2.get().setRawData(bArr);
        }
        return ofNullable2;
    }
