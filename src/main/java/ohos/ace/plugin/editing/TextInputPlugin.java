...
public class TextInputPlugin extends TextInputPluginBase {
    ...
    public interface Listener {
        ...
        void onInited(int i, EditableText editableText, EditorAttribute editorAttribute);
    }

    public TextInputPlugin(Listener listener2, int i) {
        super(i);
    ...
    public void showTextInput(boolean z) {
        ...
        if (this.isInputTypeChanged) {
            ...
            inputMethodController.startInput(0, true);
            return;
        }
        inputMethodController.startInput(0, z);
    }
    ...
    public void hideTextInput() {
        InputMethodController inputMethodController = this.imc;
        if (inputMethodController != null) {
            inputMethodController.stopInput(0);
        }
    }
    ...
    public void onInited() {
        ...
        if (this.listener != null) {
            EditorAttribute createInputAttribute = createInputAttribute(getConfiguration());
    ...
    private EditorAttribute createInputAttribute(TextInputConfiguration textInputConfiguration) {
        EditorAttribute editorAttribute = new EditorAttribute();
        if (textInputConfiguration != null) {
            editorAttribute.setInputPattern(inputTypeFromTextInputType(textInputConfiguration.getType()));
            editorAttribute.setInputOption(16);
            if (textInputConfiguration.getType() == TextInputType.MULTILINE) {
                editorAttribute.setInputOption(editorAttribute.getInputOption() | 1);
            }
            if (textInputConfiguration.getAction() != TextInputAction.UNSPECIFIED) {
                editorAttribute.setEnterKeyType(convertInputAction(textInputConfiguration.getAction()));
            }
        }
