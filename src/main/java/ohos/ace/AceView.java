...
import com.huawei.ace.adapter.AceWindowBlurAdapter;
...
public class AceView extends AceAGPEngineAdapter implements IAceView {
    ...
    private AceWindowBlurAdapter aceWindowBlurAdapter;
    ...
    public AceView(Context context, int i, float f) {
        ...
        nativeSetCallback(this.mNativeViewPtr, this);
        this.resRegister = new AceResourceRegister();
        initResRegister();
        this.aceWindowBlurAdapter = new AceWindowBlurAdapter(context.getHostContext());
    }
    ...
    public void updateWindowBlurRegion(float[][] fArr) {
        this.aceWindowBlurAdapter.updateWindowBlurRegion(fArr, this.mMetrics.physicalPaddingLeft, this.mMetrics.physicalPaddingTop);
    }

    public void updateWindowDrawOp() {
        this.aceWindowBlurAdapter.updateWindowDrawOp(true);
    }
    ...
    public void initResRegister() {
        ...
        if (j != 0) {
            ...
            this.resRegister.registerPlugin(AceTexturePlugin.createRegister(new IAceTexture() {
                /* class ohos.ace.AceView.AnonymousClass1 */
    ...
    public boolean processTouchEventInner(TouchEvent touchEvent) {
        ...
        ByteBuffer processTouchEvent = AceEventProcessor.processTouchEvent(touchEvent, new TouchInfo(0.0f, 0.0f, 1.0f, 1.0f));
    ...
    public void viewCreated() {
        ...
        if (aceContextAdapter != null && !aceContextAdapter.invalid()) {
            if (this.windowModal == 0) {
                this.contextAdapter.addAnimateView(nativeGetBackgroundColor(this.mNativeViewPtr));
            }
            this.contextAdapter.setOnApplyWindowInsetsListener(new ViewportMetricsAdapter(), new Consumer() {
                /* class ohos.ace.$$Lambda$AceView$MggeqGJo69ZD3uJhLqbMx1pGeW0 */
            ...
            if (i == 1 || i == 3) {
                this.contextAdapter.setOnHomeKeyPressedListener(new AceContextAdapter.HomeKeyPressedCallback() {
                    /* class ohos.ace.AceView.AnonymousClass2 */
