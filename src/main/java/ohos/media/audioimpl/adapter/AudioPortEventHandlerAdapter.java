...
public class AudioPortEventHandlerAdapter {
    ...
    private static final ArrayMap<AudioDevicePortChangeObserver, Object> Z_2_A_LISTENER_MAP = new ArrayMap<>();
    ...
    public void registerListener(AudioDevicePortChangeObserver audioDevicePortChangeObserver) {
        synchronized (Z_2_A_LISTENER_MAP) {
            ...
            Z_2_A_LISTENER_MAP.put(audioDevicePortChangeObserver, newProxyInstance);
            try {
    ...
    public void unregisterListener(AudioDevicePortChangeObserver audioDevicePortChangeObserver) {
        synchronized (Z_2_A_LISTENER_MAP) {
            Object obj = Z_2_A_LISTENER_MAP.get(audioDevicePortChangeObserver);
            try {
                unregisterListener.invoke(audioPortEventHandler, obj);
            } catch (IllegalAccessException | InvocationTargetException e) {
                LOGGER.error("unregisterListener-invoke exception: %{public}s", e.toString());
                e.printStackTrace();
            }
            Z_2_A_LISTENER_MAP.remove(audioDevicePortChangeObserver);
        }
    }
}
