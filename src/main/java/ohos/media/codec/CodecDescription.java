...
public final class CodecDescription {
    ...
    CodecDescription(CodecDescriptionInternal codecDescriptionInternal) {
        ...
        for (String str : supportedTypes) {
            if (str.startsWith("audio") || str.startsWith(BigReportKeyValue.TYPE_VIDEO)) {
                this.mMimeTypes[i] = str;
                i++;
            } else if (str.equals("image/vnd.android.heic")) {
    ...
    private String getCodecName() {
        ...
        if ((this.mFlags & 4) != 0) {
            ...
            if (this.mOriginalName.toLowerCase().contains("omx")) {
                str = str + OMX_CODEC;
            }
        }
        if ((this.mFlags & 1) != 0) {
    ...
    public boolean isFormatSupported(Format format) {
        CodecDescriptionInternal.CodecAbilities abilitiesForType;
        String stringValue = format.getStringValue(Format.MIME);
        String[] supportedTypes = this.mInternalDescription.getSupportedTypes();
        for (String str : supportedTypes) {
            if (str.equalsIgnoreCase(stringValue) && (abilitiesForType = this.mInternalDescription.getAbilitiesForType(str)) != null && abilitiesForType.isFormatSupported(format)) {
                return true;
            }
        }
        return false;
    ...
    public Format createAudioFormat() {
        CodecDescriptionInternal.AudioAbilities audioAbilities;
        ...
        if (!(abilitiesForType == null || (audioAbilities = abilitiesForType.getAudioAbilities()) == null)) {
    ...
    public Format createSubtitleFormat(String str, String str2) {
        ...
        format.putStringValue("language", str2);
        return format;
    }
