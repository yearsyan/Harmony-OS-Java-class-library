...
public class TextDetector extends VisionBase implements ITextDetector {
    ...
    @Override // ohos.ai.cv.text.ITextDetector
    ...
    public int getApiId() {
        switch (this.textConfiguration.getDetectType()) {
            case TextDetectType.TYPE_TEXT_DETECT_SCREEN_SHOT:
                return PluginId.CV_OCR_SCREENSHOT;
            case 196610:
            case TextDetectType.TYPE_TEXT_DETECT_FOCUS_SHOOT:
            case TextDetectType.TYPE_TEXT_DETECT_FOCUS_SHOOT_EF:
            default:
    ...
    private void setWidthAndHeight(PixelMap pixelMap, Bundle bundle) {
        if (pixelMap != null) {
            Size size = pixelMap.getImageInfo().size;
            bundle.putInt(ParamKey.ORIGIN_WIDTH, size.width);
            bundle.putInt(ParamKey.ORIGIN_HEIGHT, size.height);
            return;
        }
        bundle.putInt(ParamKey.ORIGIN_WIDTH, 0);
        bundle.putInt(ParamKey.ORIGIN_HEIGHT, 0);
    }
}
