...
public class WorkSchedulerService {
    ...
    public void start() {
        ...
        if (this.context == null) {
            HiLog.error(LOG_LABEL, "Service is not instantiated, start failed!!", new Object[0]);
        } else if (!initServiceEnvironment()) {
            HiLog.error(LOG_LABEL, "Service is not ready to add in SystemAbility, start failed!!", new Object[0]);
        } else {
            int addSysAbility = SysAbilityManager.addSysAbility(1904, this.workSched.asObject());
            HiLog.info(LOG_LABEL, "Add SystemAbility result is %{public}d", Integer.valueOf(addSysAbility));
        }
    }
    ...
    public final class StubInterface extends WorkSchedulerServiceSkeleton {
        ...
        public boolean startWorkNow(WorkInfo workInfo, boolean z) throws RemoteException {
            ...
            HiLog.info(WorkSchedulerService.LOG_LABEL, "start work: %{public}s, %{private}d.", workInfo.getBundleName(), Integer.valueOf(workInfo.getCurrentWorkID()));
        ...
        public boolean stopWork(WorkInfo workInfo, boolean z, boolean z2) throws RemoteException {
            ...
            HiLog.info(WorkSchedulerService.LOG_LABEL, "stop work: %{public}s, %{private}d.", workInfo.getBundleName(), Integer.valueOf(workInfo.getCurrentWorkID()));
            int callingUid = IPCSkeleton.getCallingUid();
            int userIdFromUid = WorkSchedulerCommon.getUserIdFromUid(callingUid);
            if (WorkSchedulerCommon.checkClientPermission(workInfo, callingUid)) {
        ...
        public WorkInfo getWorkStatus(int i) throws RemoteException {
            ...
            int callingUid = IPCSkeleton.getCallingUid();
            int userIdFromUid = WorkSchedulerCommon.getUserIdFromUid(callingUid);
            HiLog.info(WorkSchedulerService.LOG_LABEL, "get work: %{public}d, %{private}d, %{public}d.", Integer.valueOf(callingUid), Integer.valueOf(i), Integer.valueOf(userIdFromUid));
        ...
        public boolean stopAndClearWorks() throws RemoteException {
            ...
            for (WorkInfo workInfo : obtainAllWorks()) {
                if (workInfo != null) {
                    HiLog.debug(WorkSchedulerService.LOG_LABEL, "stopAndCancelAllWork: %{private}d", Integer.valueOf(workInfo.getCurrentWorkID()));
