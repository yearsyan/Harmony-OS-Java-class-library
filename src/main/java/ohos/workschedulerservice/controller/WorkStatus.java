...
public final class WorkStatus {
    ...
    public void markCurrentState(int i) {
        if (i >= 0 && i <= 5) {
            ...
            if (this.workRunningState == 1 && this.startTrackingTime != 0 && (i == 2 || i == 3)) {
                ...
                updatePriority(ceil <= 5 ? 10 : ceil <= 50 ? -10 : ceil <= 80 ? -30 : ceil < 100 ? -50 : -90);
                this.startTrackingTime = 0;
            }
            ...
            HiLog.debug(LOG_LABEL, "update state: %{public}d %{private}d, new state %{public}d, priority %{public}d !", Integer.valueOf(this.uid), Integer.valueOf(this.work.getCurrentWorkID()), Integer.valueOf(i), Integer.valueOf(this.workPriority));
        }
    }
    ...
    public boolean changeStorageSatisfiedCondition(boolean z) {
        boolean z2 = false;
        if ((this.work.getRequestStorageType() == 1) == z) {
            z2 = true;
        }
        return changeSatisfiedCondition(32, z2);
    }

    public boolean changeIdleCondition(boolean z) {
        return changeSatisfiedCondition(2, this.work.isRequestDeepIdle() == z);
    }

    public boolean changeNetWorkSatisfiedCondition(int i) {
        return changeSatisfiedCondition(1, (i & this.work.getRequestNetworkType()) != 0);
    }

    public boolean changeChargingSatisfiedCondition(boolean z, int i) {
        return changeSatisfiedCondition(4, (i & this.work.getRequestChargeType()) != 0 && z);
    }
    ...
    public boolean changeBatteryLevelSatisfiedCondition(int i) {
        ...
        return changeSatisfiedCondition(16, z);
    }
    ...
    public boolean isReady() {
        ...
        HiLog.debug(LOG_LABEL, "WorkID: %{private}s, isReady: %{public}s", Integer.valueOf(this.work.getCurrentWorkID()), Boolean.valueOf(z));
        return z;
    }
