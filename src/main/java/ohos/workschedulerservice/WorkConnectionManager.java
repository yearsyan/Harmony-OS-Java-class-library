            ...
            if (this.workServiceConnections.size() < this.maxConnectionSize) {
                if (this.workServiceConnections.get(workStatus) == null) {
                    ...
                    if (workServiceConnection.onWorkStartEvent()) {
                        BackgroundTaskManagerInner.noteWorkStart(workStatus.getUid(), workStatus.getBundleName());
    ...
    public void cleanConnection(WorkStatus workStatus) {
        synchronized (this.lock) {
            ...
            if (workServiceConnection != null) {
                ...
                BackgroundTaskManagerInner.noteWorkStop(workStatus.getUid(), workStatus.getBundleName());
                ...
                if (i != 1) {
                    if (i != 2) {
                        if (i != 3) {
                            ...
                            HiLog.info(WorkConnectionManager.LOG_LABEL, "BIND_FAILED event called! workid:%{private}d", Integer.valueOf(workStatus.getWorkId()));
                        ...
                        HiLog.info(WorkConnectionManager.LOG_LABEL, "DESTORY_CONNECTION event called! workid:%{private}d", Integer.valueOf(workStatus2.getWorkId()));
                    ...
                    HiLog.info(WorkConnectionManager.LOG_LABEL, "EXECUTE_TIMEOUT event called! workid:%{private}d", Integer.valueOf(workStatus3.getWorkId()));
