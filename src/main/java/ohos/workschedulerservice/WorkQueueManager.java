...
import ohos.backgroundtaskmgr.BackgroundTaskManagerInner;
...
public class WorkQueueManager implements IDeviceStateMonitor {
    ...
    private long calculateDelayTime(int i) {
        ...
        if (i == 1) {
            return WorkStatus.WORKING_DELAY_TIME;
        }
        if (i == 2) {
            return WorkStatus.FREQUENT_DELAY_TIME;
        }
        if (i == 3) {
            return 86400000;
        }
        return WorkStatus.NEVER_DELAY_TIME;
    }
    ...
    public void onCommonEventChanged(Intent intent, ArrayList<CommonEventStatus> arrayList) {
        ...
        HiLog.debug(LOG_LABEL, "onCommonEventChanged %{public}s!", action);
    ...
    static /* synthetic */ void lambda$tryGetAllWorkStatus$0(List list, WorkStatus workStatus) {
        if (workStatus != null) {
            HiLog.debug(LOG_LABEL, "tryGetAllWorksStatus begin, workId is %{private}d", Integer.valueOf(workStatus.getWorkId()));
    ...
    private String getBundleName(int i) {
        List<String> bundleNameFormUid = WorkSchedulerCommon.getBundleNameFormUid(i);
        return (bundleNameFormUid == null || bundleNameFormUid.isEmpty()) ? "invalid name" : bundleNameFormUid.get(0);
    }
    ...
    private void checkWorkState(WorkStatus workStatus, long j) {
        if (workStatus != null) {
            ...
            if (workStatus.getCurrentState() == 2) {
                ...
                HiLog.info(LOG_LABEL, "work is cancel, no need check!", new Object[0]);
        ...
        if (WorkConnectionManager.getInstance().hasConnections()) {
            ...
            while (it.hasNext()) {
                ...
                if (!(next.getCurrentState() == 1 || next.getCurrentState() == 3)) {
                    updateMaxConnections();
                    if (!checkReadyAgain(next) || !isAllowed(next)) {
                        HiLog.debug(LOG_LABEL, "%{public}s is not allowed or not ready again", next.getBundleName());
                        arrayList.add(next);
                    } else {
    ...
    private boolean isAllowed(WorkStatus workStatus) {
        String bundleName = workStatus.getBundleName();
        return !isForbidden(bundleName) && BackgroundTaskManagerInner.isWorkScheduleAllowed(workStatus.getUid(), bundleName, workStatus.hasBatteryCondition() ? 1 : 0);
    }
    ...
    public final class WorkQueue {
        ...
        public boolean add(WorkStatus workStatus, int i) {
            ...
            synchronized (WorkQueueManager.this.lock) {
                ...
                if (workStatusById != null || orDefault.size() < 50) {
    ...
    private void dumpEx(java.io.PrintWriter r10, java.lang.String[] r11) {
        /*
        // Method dump skipped, instructions count: 174
        */
        throw new UnsupportedOperationException("Method not decompiled: ohos.workschedulerservice.WorkQueueManager.dumpEx(java.io.PrintWriter, java.lang.String[]):void");
    }
    ...
    private void dumpForbidden(PrintWriter printWriter) {
        List<String> appForbidden = WorkschedulerAdapter.getAppForbidden();
        printWriter.println("dumpForbidden:");
