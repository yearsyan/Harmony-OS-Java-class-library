...
public class CategoryOrientationData extends SensorData<CategoryOrientation> {
    ...
    public static void getDeviceRotationMatrix(float[] fArr, float[] fArr2) {
        if (!isParameterValid(fArr, fArr2)) {
            HiLog.error(LABEL, "getDeviceRotationMatrix rotationVector length or rotationMatrix length is invalid", new Object[0]);
        } else {
    ...
    private static boolean isParameterValid(float[] fArr, float[] fArr2) {
        if (fArr2.length < 3) {
            HiLog.error(LABEL, "isParameterValid rotationVector length is %{public}d", Integer.valueOf(fArr2.length));
            return false;
        } else if (fArr.length == 9 || fArr.length == 16) {
            return true;
        } else {
            HiLog.error(LABEL, "isParameterValid rotationMatrix length is %{public}d", Integer.valueOf(fArr.length));
            return false;
        }
    }
    ...
    private static float[] getThreeDimensionalRotationMatrix(float[] fArr) {
        ...
        if (fArr.length < 4) {
            HiLog.error(LABEL, "getRotationMatrix rotationVector length is %{public}d", Integer.valueOf(fArr.length));
        } else {
