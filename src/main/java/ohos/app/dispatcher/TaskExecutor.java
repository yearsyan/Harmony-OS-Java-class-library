...
public class TaskExecutor extends WorkerPool implements DelayExecuteService, WorkerThread.Delegate {
    ...
    public boolean delayExecute(Runnable runnable, long j) {
        if (j <= 0) {
            ...
            this.delayTasks.offer(new DelayTaskWrapper(runnable, j));
            ...
            synchronized (this.consumerLock) {
                if (this.consumer == null) {
                    ...
                    thread.setUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        /* class ohos.app.dispatcher.TaskExecutor.AnonymousClass2 */

                        public void uncaughtException(Thread thread, Throwable th) {
                            AppLog.w("TaskExecutor.ensureConsumeStarted catch exception, error = %{public}s", th.getMessage());
                        }
                    });
                    thread.start();
