...
public final class NetManager {
    ...
    public boolean addDefaultNetStatusCallback(NetStatusCallback netStatusCallback) {
        ...
        try {
            NetRemoteEvent netRemoteEvent = new NetRemoteEvent(ICourierEx.MESSENGER_DESCRIPTOR);
            netRemoteEvent.setEventHandler(getDefaultHandler());
            netStatusCallback.networkRequest = this.mNetManagerProxy.addDefaultNetStatusCallback(netRemoteEvent);
            if (netStatusCallback.networkRequest != null) {
                HiLog.warn(LABEL, "requestId: %{public}d", Integer.valueOf(netStatusCallback.networkRequest.requestId));
                NET_CALLBACKS.put(netStatusCallback.networkRequest, netStatusCallback);
            }
    ...
    @SystemApi
    public boolean enableDistributedCellularData(boolean z) {
        try {
            return this.mNetManagerProxy.enableDistributedCellularData(z);
        } catch (RemoteException unused) {
            HiLog.warn(LABEL, "Failed to enableDistributedCellularData", new Object[0]);
            return false;
        }
    }
}
