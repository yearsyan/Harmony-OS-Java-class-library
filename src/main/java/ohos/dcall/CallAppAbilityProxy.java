...
public class CallAppAbilityProxy {
    static final Integer INVALID_KEY_SET_IN_MAP = -1;
    ...
    private CallAppAbilityConnection.Listener mListener = new CallAppAbilityConnection.Listener() {
        ...
        public void onReleased(CallAppAbilityConnection callAppAbilityConnection) {
            if (callAppAbilityConnection != null && callAppAbilityConnection.getInfo() != null) {
                HiLog.info(CallAppAbilityProxy.LOG_LABEL, "Connection is released %{public}s.", callAppAbilityConnection.toString());
    ...
    public boolean preOnCallAdded(String str, int i, int i2, String str2, String str3) {
        CallAppAbilityConnection callAppAbilityConnection = null;
        for (Map.Entry<Integer, CallAppAbilityConnection> entry : this.mCallAppAbilityConnectionMap.entrySet()) {
            Integer key = entry.getKey();
            HiLog.info(LOG_LABEL, "preOnCallAdded: type %{public}d.", key);
            CallAppAbilityConnection callAppAbilityConnection2 = this.mCallAppAbilityConnectionMap.get(key);
            if (callAppAbilityConnection2 != null) {
                if (callAppAbilityConnection2.hasLiveCalls()) {
                    return false;
                }
                if (key.intValue() == i2) {
                    callAppAbilityConnection = callAppAbilityConnection2;
                }
            }
        }
        if (callAppAbilityConnection == null) {
            callAppAbilityConnection = createConnectionByCallType(i2);
        }
        if (callAppAbilityConnection != null) {
            return callAppAbilityConnection.preOnCallAdded(str, i, str2, str3);
        }
        return false;
    }
    ...
    public CallAppAbilityInfo getDefaultCallAppAbilityInfoByCallType(int i) {
        HiLog.info(LOG_LABEL, "getDefaultCallAppAbilityInfoByCallType:call type is %{public}d.", Integer.valueOf(i));
        if (i == 1) {
    ...
    public CallAppAbilityConnection getConnectionByCallType(int i) {
        ...
        HiLog.info(LOG_LABEL, "getConnectionByCallType, call type is %{public}d.", Integer.valueOf(i));
        for (Map.Entry<Integer, CallAppAbilityConnection> entry : this.mCallAppAbilityConnectionMap.entrySet()) {
            Integer key = entry.getKey();
            HiLog.info(LOG_LABEL, "getConnectionByCallType: type %{public}d.", key);
            if (key.intValue() == i && (callAppAbilityConnection = this.mCallAppAbilityConnectionMap.get(key)) != null) {
                return callAppAbilityConnection;
            }
        }
        return createConnectionByCallType(i);
    }

    /* access modifiers changed from: package-private */
    public CallAppAbilityConnection createConnectionByCallType(int i) {
        ...
        CallAppAbilityConnection callAppAbilityConnection = new CallAppAbilityConnection(this.mContext, this.mLock, defaultCallAppAbilityInfoByCallType);
        callAppAbilityConnection.setListener(this.mListener);
        this.mCallAppAbilityConnectionMap.put(Integer.valueOf(i), callAppAbilityConnection);
        return callAppAbilityConnection;
    }
