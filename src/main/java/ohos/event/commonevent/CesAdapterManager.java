...
public class CesAdapterManager {
    ...
    public void onReceiveCommonEvent(Intent intent, int i, String str, boolean z, int i2) {
        C0000Bytrace.startTrace(2, "JavaSubscriberOnReceive2");
        if (intent != null) {
            if (!nativeAvailable) {
                HiLog.error(LOG_LABEL, "onReceiveCommonEvent, libces_jni.z.so is not available", new Object[0]);
                return;
            }
            HiLog.debug(LOG_LABEL, "onReceiveCommonEvent action=%{public}s", intent.getAction());
            ...
            C0000Bytrace.finishTrace(2, "JavaSubscriberOnReceive2");
        }
    }
    ...
    public void onResultReceiverReceive(long j, Intent intent, int i, String str) {
        if (intent != null) {
            ...
            HiLog.debug(LOG_LABEL, "onResultReceiverReceive action=%{public}s", intent.getAction());
            Optional<Parcel> createEventParcel = createEventParcel(intent, i, str);
            if (!createEventParcel.isPresent()) {
    ...
    private int handleSubscribeCommonEvent(long j) {
        ...
        C0000Bytrace.startTrace(2, "JavaSerSubscribe");
        ...
        if (!registerReceiver(commonEventSubscribeInfo, cesProxyReceiver, createAndroidIntentFilter.get(), str)) {
            ...
            C0000Bytrace.finishTrace(2, "JavaSerSubscribe");
            return 0;
        }
    ...
    private void handleUnsubscribeCommonEvent(int i) {
        ...
        C0000Bytrace.startTrace(2, "JavaSerunSubscribe");
        this.globalContext.unregisterReceiver(remove);
        C0000Bytrace.finishTrace(2, "JavaSerunSubscribe");
    }
