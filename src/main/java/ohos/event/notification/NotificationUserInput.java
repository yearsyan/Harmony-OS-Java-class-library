...
public class NotificationUserInput implements Sequenceable {
    ...
    public static void addMimeInputToIntent(NotificationUserInput notificationUserInput, Intent intent, Map<String, Uri> map) {
        ...
        PacMap pacMap2 = (PacMap) intent.getSequenceableParam(NOTIFICATION_USER_INPUT);
        if (pacMap2 == null) {
            pacMap2 = new PacMap();
        }
        for (Map.Entry<String, Uri> entry : map.entrySet()) {
            ...
            if (!(key == null || (value = entry.getValue()) == null)) {
                PacMap orElse = pacMap2.getPacMap(getExtraInputsKeyForData(key)).orElse(new PacMap());
                orElse.putString(notificationUserInput.getInputKey(), value.toString());
                pacMap2.putPacMap(getExtraInputsKeyForData(key), orElse);
            }
        }
        intent.setParam(NOTIFICATION_USER_INPUT, pacMap2);
    }
    ...
    public static void addInputsToIntent(List<NotificationUserInput> list, Intent intent, PacMap pacMap2) {
        ...
        PacMap pacMap3 = (PacMap) intent.getSequenceableParam(NOTIFICATION_USER_INPUT);
        if (pacMap3 == null) {
            pacMap3 = new PacMap();
        }
        for (NotificationUserInput notificationUserInput : list) {
            if (!(notificationUserInput == null || (string = pacMap2.getString(notificationUserInput.getInputKey())) == null)) {
                pacMap3.putString(notificationUserInput.getInputKey(), string);
            }
        }
        intent.setParam(NOTIFICATION_USER_INPUT, pacMap3);
    }
    ...
    public static Map<String, Uri> getMimeInputsFromIntent(Intent intent, String str) {
        ...
        for (String str2 : pacMap2.getKeys()) {
            if (str2 != null && str2.startsWith(EXTRA_RESULTS_DATA_TYPE)) {
                String substring = str2.substring(39);
                if (!substring.isEmpty()) {
                    Optional<PacMap> pacMap3 = pacMap2.getPacMap(str2);
                    if (pacMap3.isPresent() && (string = pacMap3.get().getString(str)) != null && !string.isEmpty()) {
    ...
    public static PacMap getInputsFromIntent(Intent intent) {
        ...
        return (PacMap) intent.getSequenceableParam(NOTIFICATION_USER_INPUT);
    }
