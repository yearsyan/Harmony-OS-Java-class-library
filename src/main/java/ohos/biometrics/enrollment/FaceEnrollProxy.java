...
public class FaceEnrollProxy implements IFaceEnroll {
        ...
        public void onCallbackEvent(int i, int i2, int i3, int i4) {
            if (i == FaceEnrollProxy.this.mReqId && i2 == 1) {
                ...
                if (i3 == 1 && FaceEnrollProxy.this.mSignal != null) {
                    HiLog.info(FaceEnrollProxy.LABEL, "get enroll result from callback event! errorCode = %{public}d", Integer.valueOf(i4));
                    FaceEnrollProxy.this.mResult = i4;
                    FaceEnrollProxy.this.mSignal.countDown();
                }
                if (i3 == 6) {
                    HiLog.info(FaceEnrollProxy.LABEL, "get enroll face id from callback event! faceid = ****%{public}04x", Integer.valueOf(65535 & i4));
                    FaceEnrollProxy.this.setEnrolledFaceId(i4);
                    return;
                }
                return;
            }
            HiLog.info(FaceEnrollProxy.LABEL, "callback event ignored: reqId = %{public}d, type = %{public}d", Integer.valueOf(i), Integer.valueOf(i2));
        }
    };
    ...
    public BiometricEnroll.EnrollInitResult initEnroll() {
        ...
        if (init != 0) {
            HiLog.error(LABEL, "initAlgo failed: %{public}d", Integer.valueOf(init));
            return enrollInitResult;
        }
    ...
    public int execEnroll(byte[] bArr, long j, int i, Surface surface) {
        ...
        int i2 = this.mReqId;
        if (((int) j) != i2) {
            HiLog.error(LABEL, "sessionId error: %{public}d", Long.valueOf(j));
            return -1;
        }
        ...
        if (enroll != 0) {
            HiLog.error(LABEL, "mFaceManagerImpl.enroll failed: %{public}d", Integer.valueOf(enroll));
            return -1;
        }
        try {
            this.mSignal.await();
        ...
        HiLog.info(LABEL, "execEnroll end: result = %{public}d", Integer.valueOf(this.mResult));
        return this.mResult;
    }
    ...
    public int finishEnroll() {
        ...
        HiLog.error(LABEL, "releaseAlgo failed: %{public}d", Integer.valueOf(release));
        return -1;
    }
    ...
    public List<BiometricEnroll.EnrollInfo> getEnrolledInfo() {
        ...
        for (int i : enrolledFaceIDs) {
            ...
            if (faceInfo != null) {
                ...
                HiLog.error(LABEL, "getFaceInfo null! faceId = %{public}d", Integer.valueOf(i));
            }
        }
        return arrayList;
    }
