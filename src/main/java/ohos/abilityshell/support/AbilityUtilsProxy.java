...
public class AbilityUtilsProxy {
    ...
    public boolean freeInstallWithCallback(Context context, ohos.aafwk.content.Intent intent, IRemoteObject iRemoteObject) {
        IAbilityUtils service = AbilityUtilsHelper.getService();
        if (service != null) {
            return service.freeInstallWithCallback(context, intent, iRemoteObject);
        }
        AppLog.e(LOG_LABEL, "AbilityUtilsProxy::freeInstallWithCallback can not get IAbilityUtils service!", new Object[0]);
        return false;
    }
