...
public class DistributedImpl implements IDistributedManager {
    ...
    public int startRemoteFreeInstallAbility(Intent intent, IRemoteObject iRemoteObject) throws RemoteException {
        synchronized (LOCK) {
            if (!initDistributedServiceProxy()) {
                AppLog.e(SHELL_LABEL, "DistributedImpl::startRemoteFreeInstallAbility get proxy failed", new Object[0]);
                return -1;
            }
            return distributedMgrProxy.startRemoteFreeInstallAbility(intent, iRemoteObject);
        }
    }
