...
public class ContinuationHandler implements ContinuationSchedulerForDmsStub.IDistributeScheduleHandler, IReverseContinuationSchedulerSlave, IReverseContinuationSchedulerMaster {
    ...
    public int handleStartContinuation(IRemoteObject iRemoteObject, String str) {
        ...
        Integer num = null;
        try {
            num = this.distManager.startContinuation(setIntentParams(intentParams), this.abilityInfo, iRemoteObject);
    ...
    private Intent setIntentParams(IntentParams intentParams) {
    ...
    private String getLocalDeviceId() {
        ...
        Optional<BasicInfo> localBasicInfo = new DeviceManager().getLocalBasicInfo();
        if (localBasicInfo.isPresent()) {
            this.localDeviceId = localBasicInfo.get().getNodeId();
    ...
    public void handleReceiveRemoteScheduler(IRemoteObject iRemoteObject) {
        ...
        IReverseContinuationSchedulerMaster.ReverseContinuationSchedulerMasterStub reverseContinuationSchedulerMasterStub = this.masterStub;
        if (reverseContinuationSchedulerMasterStub == null) {
            AppLog.e(LABEL, "handleReceiveRemoteScheduler: Master stub not initialized, skip send to slave", new Object[0]);
        } else {
            this.slaveProxy.passMaster(reverseContinuationSchedulerMasterStub);
        }
    }
