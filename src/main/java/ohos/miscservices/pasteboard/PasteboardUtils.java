...
public class PasteboardUtils {
    ...
    public static ClipData.Item convertFromRecord(PasteData.Record record) {
        ...
        if (c == 2) {
            Optional<Intent> createAndroidIntent = IntentConverter.createAndroidIntent(record.getIntent(), null);
            if (createAndroidIntent.isPresent()) {
                intent = createAndroidIntent.get();
            }
    ...
    public static List<PasteData.Record> convertFromItem(ClipData.Item item) {
        ...
        if (item.getIntent() != null) {
            ...
            Optional<ohos.aafwk.content.Intent> createZidaneIntent = IntentConverter.createZidaneIntent(item.getIntent(), null);
            if (createZidaneIntent.isPresent()) {
                intent = createZidaneIntent.get();
            }
