...
public class SqliteSharedResultSet extends AbsSharedResultSet {
    ...
    private void fillBlock(int i) {
        ...
        try {
            if (this.rowNum == -1) {
                ...
                HiLog.info(LABEL, "fillBlock received count(*) from native_fill_window: %{public}d", Integer.valueOf(this.rowNum));
                return;
            }
            ...
            HiLog.info(LABEL, "fillBlock startPos = %{public}d, requiredPos = %{public}d", Integer.valueOf(i2), Integer.valueOf(i));
    ...
    @Override // ohos.data.resultset.AbsResultSet, ohos.data.resultset.AbsSharedResultSet
    public void finalize() {
        try {
            if (!isClosed()) {
                RdbStoreImpl rdbStore = this.query.getRdbStore();
                if (rdbStore.isOpen()) {
                    HiLog.info(LABEL, "Finalizing a shared block that has not been closed, database name is %{public}s", rdbStore.isMemoryRdb() ? SqliteDatabaseConfig.MEMORY_DB_PATH : new File(rdbStore.getPath()).getName());
                }
                close();
            }
        } finally {
