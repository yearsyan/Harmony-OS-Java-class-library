...
public class QueryStatement extends SqlStatement implements Query {
    ...
    public PrecompiledStatement beginStepQuery() {
        try {
            ...
            HiLog.error(LABEL, "database corrupt: %{public}s; beginStepQuery: %{private}s", e.getMessage(), this.getSql());
    ...
    public int step() {
        ...
        try {
            ...
            HiLog.error(LABEL, "database corrupt: %{public}s; query: %{private}s", e.getMessage(), getSql());
            getRdbStore().handleCorruption();
            throw e;
        } catch (RdbException e2) {
            HiLog.error(LABEL, "exception: %{public}s; query: %{private}s", e2.getMessage(), getSql());
            throw e2;
        } catch (Throwable th) {
            releaseRef();
            throw th;
    ...
    public void endStepQuery(PrecompiledStatement precompiledStatement) {
        try {
            ...
            HiLog.error(LABEL, "database corrupt: %{public}s; endStepQuery: %{private}s", e.getMessage(), getSql());
            getRdbStore().handleCorruption();
            throw e;
        }
    }
    ...
    public void resetStatement(PrecompiledStatement precompiledStatement) {
        try {
            ...
            HiLog.error(LABEL, "database corrupt: %{public}s; reset: %{private}s", e.getMessage(), getSql());
            getRdbStore().handleCorruption();
            throw e;
        }
    }
    ...
    public int fillBlock(SharedBlock sharedBlock, int i, int i2, boolean z) {
        ...
        try {
            ...
            try {
                ...
                HiLog.error(LABEL, "database corrupt: %{public}s; fillBlock: %{private}s", e.getMessage(), getSql());
                getRdbStore().handleCorruption();
                throw e;
            } catch (RdbException e2) {
                HiLog.error(LABEL, "exception: %{public}s; query: %{private}s", e2.getMessage(), getSql());
