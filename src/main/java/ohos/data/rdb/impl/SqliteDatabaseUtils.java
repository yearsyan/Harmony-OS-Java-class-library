...
public class SqliteDatabaseUtils {
    ...
    public static void deleteFile(String str) {
        File file = new File(str);
        if (!file.exists()) {
            HiLog.info(LABEL, "File %{private}s does not exist", str);
        } else if (file.delete()) {
            HiLog.info(LABEL, "FileName= %{private}s has been deleted", str);
        } else {
            HiLog.info(LABEL, "Failed to delete File %{private}s", str);
        }
    }
    ...
    public static boolean renameFile(String str, String str2) {
        ...
        if (!file.exists()) {
            ...
            HiLog.info(LABEL, "Rename oldFileName = %{private}s to newFileName  %{private}s", str, str2);
            return true;
        }
    }
    ...
    public static File getCorruptPath(String str) {
        ...
        boolean endsWith = name2.endsWith("_encrypt");
        StringBuilder sb = new StringBuilder();
        sb.append(DatabaseFileType.CORRUPT.getValue());
        sb.append(endsWith ? "_encrypt" : "");
    ...
    public static File getDatabasePath(Context context, DatabaseFileConfig databaseFileConfig) {
        if (context == null) {
            ...
            if (name == null || name.isEmpty()) {
                ...
                DatabaseFileSecurityLevel databaseFileSecurityLevel = databaseFileConfig.getDatabaseFileSecurityLevel();
                if (FileProtectHelper.isValid(context, databaseFileSecurityLevel)) {
                    File file = new File(getDatabaseRootDir(context), getDatabaseDir(databaseFileConfig, databaseFileSecurityLevel));
                    synchronized (CREATE_DIR_LOCK) {
                        if (!file.exists()) {
                            if (!file.mkdir()) {
                                throw new IllegalStateException("Create database dir failed.");
                        ...
                        FileProtectHelper.setPolicy(context, file.toString(), databaseFileSecurityLevel);
                    }
                    return new File(file, name);
                }
                throw new IllegalArgumentException("Incorrect encryption risk level for secure config.");
            } else {
                throw new IllegalArgumentException("Incorrect database name.");
            }
        } else {
    ...
    private static String getDatabaseDir(DatabaseFileConfig databaseFileConfig, DatabaseFileSecurityLevel databaseFileSecurityLevel) {
        String value = databaseFileConfig.getDatabaseFileType().getValue();
        String str = databaseFileConfig.isEncrypted() ? "_encrypt" : "";
        String prefix = FileProtectHelper.getPrefix(databaseFileSecurityLevel);
