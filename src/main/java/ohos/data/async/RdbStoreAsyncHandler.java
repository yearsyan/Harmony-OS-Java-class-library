        ...
        private AbsAsyncHandler.JobInfo processRdb(int i, AbsAsyncHandler.JobInfo jobInfo, RdbStore rdbStore) {
            ...
            if (i == 1) {
                try {
                    ...
                    HiLog.error(RdbStoreAsyncHandler.LABEL, "Async query data occur an exception %{private}s", e);
                    jobInfo.result = null;
                }
            } else if (i == 2) {
                try {
                    ...
                    HiLog.error(RdbStoreAsyncHandler.LABEL, "Async insert data occur an exception %{private}s", e2);
                    jobInfo.result = null;
                }
            } else if (i != 3) {
                try {
                    ...
                    HiLog.error(RdbStoreAsyncHandler.LABEL, "Async delete data occur an exception %{private}s", e3);
                    jobInfo.result = null;
                }
            } else {
                try {
                    ...
                    HiLog.error(RdbStoreAsyncHandler.LABEL, "Async update data occur an exception %{private}s", e4);
                    jobInfo.result = null;
                }
            }
            return jobInfo;
