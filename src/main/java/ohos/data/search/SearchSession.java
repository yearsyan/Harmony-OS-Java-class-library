...
public class SearchSession {
    ...
    public List<IndexData> search(String str, int i, int i2) {
        ...
        try {
            ...
            HiLog.error(LABEL, "Failed to search, read reply memory error, errMsg: %{public}s", e.getMessage());
        } catch (Throwable th) {
            SharedMemoryHelper.releaseMemory(null);
            throw th;
        }
        SharedMemoryHelper.releaseMemory(sharedMemory);
        HiLog.info(LABEL, "search end, hit size is %{public}d, cost %{public}d", Integer.valueOf(emptyList.size()), Long.valueOf(System.currentTimeMillis() - currentTimeMillis));
        return emptyList;
    }
    ...
    public List<Recommendation> groupSearch(String str, int i) {
        ...
        HiLog.info(LABEL, "groupSearch end, hit size is %{public}d, cost %{public}d", Integer.valueOf(innerRecommendations2Recommendations.size()), Long.valueOf(System.currentTimeMillis() - currentTimeMillis));
