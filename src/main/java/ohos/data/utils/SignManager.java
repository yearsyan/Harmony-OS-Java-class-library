...
public class SignManager {
    ...
    private static String generateSecretKeyAlias(Context context) {
        if (context != null) {
            ...
            return "database_sign_" + bundleName + "_" + "HmacSHA256";
        }
    ...
    private static SecretKey generateSecretKey(String str) {
        try {
            ...
            KeyGenerator instance2 = KeyGenerator.getInstance("HmacSHA256", "HarmonyKeyStore");
            instance2.init(new KeyGenAlgorithmParaSpec.Builder(str).setSecKeyUsagePurposes(4).createKeyGenAlgorithmParaSpec());
            ...
            HiLog.error(LABEL, "Generate secret key error. Throw %{public}s exception : %{public}s", e.getClass().getName(), e.getMessage());
    ...
    private static byte[] generateSign(SecretKey secretKey) {
        try {
            Mac instance = Mac.getInstance("HmacSHA256", "HarmonyKeyStoreBCWorkaround");
            instance.init(secretKey);
            return instance.doFinal("database_sign".getBytes(StandardCharsets.UTF_8));
        } catch (InvalidKeyException | NoSuchAlgorithmException | NoSuchProviderException e) {
            HiLog.error(LABEL, "Generate sign error. Throw %{public}s exception : %{public}s", e.getClass().getName(), e.getMessage());
