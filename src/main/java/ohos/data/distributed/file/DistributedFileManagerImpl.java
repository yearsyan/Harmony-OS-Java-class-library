...
public class DistributedFileManagerImpl implements DistributedFileManager {
    ...
    static {
        try {
            ...
            HiLog.error(LABEL, "load libdistributedfile.z.so UnsatisfiedLinkError: %{public}s", e);
        }
    }
    ...
    public String getBundleDistributedDir(String str) {
        ...
        try {
            ...
            if (nativeGetBundleDistributedDir == null) {
                HiLog.error(LABEL, "bundle %{private}s, cannot get distributed dir", str);
                return "";
            }
            HiLog.info(LABEL, "bundle %{private}s, distributed dir %{private}s", str, nativeGetBundleDistributedDir);
            return nativeGetBundleDistributedDir;
        } catch (UnsatisfiedLinkError e) {
            HiLog.error(LABEL, "nativeGetBundleDistributedDir UnsatisfiedLinkError: %{public}s", e);
            return "";
        }
    }
    ...
    public String getSystemDistributedDir(String str) {
        ...
        try {
            ...
            if (nativeGetSystemDistributedDir == null) {
                HiLog.error(LABEL, "cannot create distributed dir %{private}s", str);
                return "";
            }
            HiLog.info(LABEL, "create distributed dir %{private}s", nativeGetSystemDistributedDir);
            return nativeGetSystemDistributedDir;
        } catch (UnsatisfiedLinkError e) {
            HiLog.error(LABEL, "nativeGetSystemDistributedDir UnsatisfiedLinkError: %{public}s", e);
            return "";
        }
    }
}
