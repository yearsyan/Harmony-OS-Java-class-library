...
public class ServiceEventSubscriber extends CommonEventSubscriber {
    ...
    public void onReceiveEvent(CommonEventData commonEventData) {
        ...
        HiLog.info(LABEL, "received event action:%{public}s", action);
        if (ACTION_NAME.equals(action)) {
            ...
            HiLog.info(LABEL, "received event, start ServiceAbility, param:%{public}s.", str);
            bindService(str);
            HiLog.info(LABEL, "start ServiceAbility end.", new Object[0]);
            return;
        }
        HiLog.info(LABEL, "received event unknown action:%{public}s", action);
    }
    ...
    private void bindService(String str) {
        try {
                ...
                if (convertIntentShell.isPresent()) {
                    if (this.mContext != null) {
                        if (CONNECTIONS.get(str) != null) {
                            HiLog.warn(LABEL, "received event, repeated bind, %{public}s.", str);
                            return;
                        }
                        ...
                        if (this.mContext.bindServiceAsUser(convertIntentShell.get(), workServiceConnection, 1, UserHandle.CURRENT)) {
                            ...
                            HiLog.warn(LABEL, "received event, bind failed, %{public}s.", str);
                        }
                        return;
                    }
                }
            ...
            HiLog.error(LABEL, "start ServiceAbility exception message:%{public}s.", e.getMessage());
        }
    }
    ...
    public class WorkServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            HiLog.info(ServiceEventSubscriber.LABEL, "connected, %{public}s.", componentName.getPackageName());
            ServiceEventSubscriber.this.unbindService(componentName);
        }

        public void onServiceDisconnected(ComponentName componentName) {
            HiLog.info(ServiceEventSubscriber.LABEL, "disconnected, %{public}s.", componentName.getPackageName());
        }
    }
}
