...
public class SingleKvStoreImpl implements SingleKvStore {
    ...
    private static native int nativeGetSecurityLevel(long j);
    ...
    @Override // ohos.data.distributed.user.SingleKvStore
    public SecurityLevel getSecurityLevel() throws KvStoreException {
        int nativeGetSecurityLevel = nativeGetSecurityLevel(this.nativeKvStore);
        SecurityLevel[] values = SecurityLevel.values();
        for (SecurityLevel securityLevel : values) {
            if (securityLevel.getCode() == nativeGetSecurityLevel) {
                return securityLevel;
            }
        }
        return SecurityLevel.NO_LEVEL;
    }
}
