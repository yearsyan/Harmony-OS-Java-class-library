...
public class StoragePlugin implements ModuleGroup.ModuleGroupHandler, ErrorCode {
    ...
    public void onFunctionCall(Function function, Result result) {
        if (result == null) {
            ...
            try {
                ...
                HiLog.error(LABEL, "IllegalArgumentException occur when %{public}s data, eMsg:%{public}s", function.name, e.getMessage());
                result.error(200, "Failed to " + function.name + " data");
            } catch (RuntimeException e2) {
                HiLog.error(LABEL, "RuntimeException occur when %{public}s data, eMsg:%{public}s", function.name, e2.getMessage());
                result.error(200, "Failed to " + function.name + " data");
            }
        }
    }
    ...
    public static void register(Context context2) {
        if (context2 == null) {
            ...
            HiLog.debug(LABEL, "register abilityId:%{public}d", Integer.valueOf(abilityId));
    ...
    public static void deregister(Context context2) {
        ...
        int abilityId = ((AceAbility) context2).getAbilityId();
        HiLog.debug(LABEL, "deregister abilityId:%{public}d", Integer.valueOf(abilityId));
