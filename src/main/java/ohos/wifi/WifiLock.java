...
public class WifiLock {
    ...
    public synchronized void acquire() throws IOException, RemoteException {
        ...
        if (this.mIsRefCounted || !this.mIsHeld) {
            try {
                ...
                if (this.mWifiLockCount < 50) {
                    this.mWifiLockCount++;
                    this.mIsHeld = true;
                } else {
                    release();
                    throw new IOException("BeyondIndex");
                }
    ...
    public String toString() {
        return "WifiLock{mTag='" + this.mTag + PatternTokenizer.SINGLE_QUOTE + ", mCaller='" + this.mCaller + PatternTokenizer.SINGLE_QUOTE + ", mLockType=" + this.mLockType + ", mIsRefCounted=" + this.mIsRefCounted + ", mIsHeld=" + this.mIsHeld + '}';
    }
}
