...
public final class SysAbilityManager {
    ...
    private static final int MAX_SYSCAP_NAME_LEN = 64;
    ...
    public static int registerSystemReadyCallback(IRemoteObject iRemoteObject) {
        try {
            return getSysAbilityManager().registerSystemReadyCallback(iRemoteObject);
        } catch (RemoteException unused) {
            HiLog.error(TAG, "registerSystemReadyCallback remote Exception", new Object[0]);
            return 1;
        }
    }

    public static int addSystemCapability(String str) {
        if (str != null && !str.isEmpty() && str.length() <= 64) {
            try {
                return getSysAbilityManager().addSystemCapability(str);
            } catch (RemoteException unused) {
                HiLog.error(TAG, "addSystemCapability remote Exception", new Object[0]);
            }
        }
        return 1;
    }

    public static boolean hasSystemCapability(String str) {
        if (str != null && !str.isEmpty() && str.length() <= 64) {
            try {
                return getSysAbilityManager().hasSystemCapability(str);
            } catch (RemoteException unused) {
                HiLog.error(TAG, "hasSystemCapability remote Exception", new Object[0]);
            }
        }
        return false;
    }

    public static List<String> getSystemAvailableCapabilities() {
        try {
            return getSysAbilityManager().getSystemAvailableCapabilities();
        } catch (RemoteException unused) {
            HiLog.error(TAG, "getSystemAvailableCapabilities remote Exception", new Object[0]);
